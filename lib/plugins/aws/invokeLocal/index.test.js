...
describe('AwsInvokeLocal', () => {
  ...
  describe('#invokeLocal()', () => {
    ...
    it('should call invokeLocalDocker if using --docker option with nodejs16.x', () => {
      awsInvokeLocal.options.functionObj.runtime = 'nodejs16.x';
      awsInvokeLocal.options.functionObj.handler = 'handler.foobar';
  ...
  describe('#invokeLocalDocker()', () => {
    ...
    it('calls docker with packaged artifact', () =>
      awsInvokeLocal.invokeLocalDocker().then(() => {
        const dockerfilePath = path.join('.serverless', 'invokeLocal', 'nodejs16.x', 'Dockerfile');
        ...
        expect(spawnExtStub.getCall(2).args).to.deep.equal([
          'docker',
          ['build', '-t', 'sls-docker-nodejs16.x', 'servicePath', '-f', dockerfilePath],
        ]);
